...
function lintOne(name: string, filePath: string): Finding[] {
  ...
  if (kb > GMAIL_CLIP_KB) {
    add('error', 'gmail-clip', 'Gmail', `${kb.toFixed(1)} KB > ${GMAIL_CLIP_KB} KB — Gmail clips the tail and hides the unsubscribe. Run build:min or trim a section.`);
  } else if (kb > GMAIL_CLIP_KB * 0.9) {
    add('warn', 'gmail-clip', 'Gmail', `${kb.toFixed(1)} KB — within 10% of the ${GMAIL_CLIP_KB} KB clip limit. Watch size as sections are added.`);
  } else {
    add('info', 'gmail-clip', 'Gmail', `${kb.toFixed(1)} KB — comfortably under the ${GMAIL_CLIP_KB} KB clip limit.`);
  }

  // ── DOCTYPE ─────────────────────────────────────────────────────────────
  if (!/^\s*<!doctype/i.test(html)) {
    add('error', 'doctype', 'all', 'Missing <!doctype> — Outlook/webmail fall into quirks mode and mis-size the layout.');
  }

  // ── Absolute / fixed positioning — unsupported almost everywhere ─────────
  const posAbs = count(html, /position\s*:\s*(absolute|fixed)/gi);
  if (posAbs > 0) {
    add('error', 'positioning', 'Gmail/Outlook', `${posAbs}× position:absolute|fixed — stripped by Gmail and ignored by Outlook; overlapped elements collapse. Use a composited image or table layout instead.`);
  }

  // ── Flex / grid — no support in Outlook, unreliable in Gmail ─────────────
  const flex = count(nonOutlook, /display\s*:\s*(inline-)?(flex|grid)/gi);
  if (flex > 0) {
    add('error', 'flex-grid', 'Outlook/Gmail', `${flex}× display:flex|grid — Outlook (Word engine) ignores it entirely and columns collapse to one stack. Use MJML columns / <table> cells.`);
  }
  ...
  if (bgUrl > 0) {
    ...
    if (hasVml) {
      add('info', 'bg-image', 'Outlook', `${bgUrl}× CSS background-image with a VML fallback present — renders in Outlook. Confirm the VML covers each one.`);
    } else {
      add('warn', 'bg-image', 'Outlook', `${bgUrl}× CSS background-image with NO VML (v:rect/v:fill) fallback — Outlook shows a blank band. Add the mso conditional or bake the image into a plain <img>.`);
    }
  }
  ...
  for (const [tag, re] of [
    ...
    if (n > 0) add('error', 'unsupported-tag', 'all', `${n}× ${tag} — stripped by most clients and a spam-filter trigger. Remove it.`);
  }
  ...
  if (nonFontLinks.length > 0) {
    add('error', 'external-css', 'Gmail/Outlook', `${nonFontLinks.length}× non-font <link rel=stylesheet> — external CSS does not load in most clients; inline the rules. (Font links are fine.)`);
  }

  // ── CSS the Word engine drops silently — informational, MJML degrades OK ─
  const radius = count(html, /border-radius/gi);
  if (radius > 0) add('info', 'outlook-degrade', 'Outlook', `${radius}× border-radius — Outlook desktop squares the corners (rest of layout unaffected). Expected.`);
  const objFit = count(html, /object-fit/gi);
  if (objFit > 0) add('info', 'outlook-degrade', 'Outlook', `${objFit}× object-fit — Outlook ignores it; a non-square source image is not cropped there. Host square crops.`);

  // ── CSS custom properties / modern colour funcs — Outlook renders black ──
  const cssVar = count(nonOutlook, /var\(--/gi);
  if (cssVar > 0) add('warn', 'css-var', 'Outlook', `${cssVar}× var(--…) custom property — Outlook desktop can't resolve it and falls back to inherited/black. Inline the literal value.`);

  // ── role=presentation on layout tables — accessibility safeguard ─────────
  const roles = count(html, /role=["']presentation["']/gi);
  add(roles > 0 ? 'info' : 'warn', 'a11y-tables', 'Screen readers', roles > 0
  ...
  add(colorScheme ? 'info' : 'warn', 'dark-mode', 'Apple Mail/iOS/Outlook.com', colorScheme
...
function main(): void {
  ...
  try {
    files = readdirSync(OUT_DIR)
      .filter((f) => f.endsWith('.html') && !f.endsWith('.preview.html') && f !== 'index.html' && f !== 'simulator.html')
      .sort();
  } catch {
